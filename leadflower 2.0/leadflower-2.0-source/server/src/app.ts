...
import messagingRoutes, { publicMessagingRouter } from './routes/messaging'
...
function mountApi(app: express.Express, prefix: '/api/v1' | '/api'): void {
  ...
  app.use(`${prefix}/sequences`, authenticate, csrfProtection, requireOrganization, tenantRateLimit, idempotentMutation, operationalViewer, mutationGate(operationalOperator), sequenceRoutes)
  // Unauthenticated by design: recipients clicking an unsubscribe link and
  // providers posting delivery events have no session. Each endpoint carries
  // its own rate limiter and derives the organisation from an unguessable
  // per-send token or from a ledger record it already wrote.
  app.use(`${prefix}/messaging`, messagingRoutes)
  app.use(`${prefix}/messaging`, publicMessagingRouter)
  ...
  app.use(`${prefix}/public/reviews`, publicReviewRouter)
  app.use(`${prefix}/forms`, authenticate, csrfProtection, requireOrganization, tenantRateLimit, idempotentMutation, operationalViewer, mutationGate(operationalOperator), formRoutes)
  // Unauthenticated by design: a hosted form is embedded on the customer's own
  // website and its submitters have no session. Addressed by an unguessable
  // slug, rate limited, and the organisation is derived from the matched form.
  app.use(`${prefix}/public/forms`, publicFormRouter)
}
