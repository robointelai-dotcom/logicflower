...
export default function TeamPage() {
  ...
  const updateRole = async (member: Member, role: OrganizationRole) => {
    await action.run(() => send('patch', `/organizations/current/members/${encodeURIComponent(member.id)}`, { role }), `${member.name ?? member.email} is now ${titleCase(role)}.`)
    await query.reload()
  }
